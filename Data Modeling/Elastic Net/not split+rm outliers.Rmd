...
library(readr)
library(glmnet)
housing_data <- read_csv("/Users/chensihan/Documents/GitHub/BIOSTAT625/Data Exploration/final_rmna_OneHotEncoded_housing.csv")
set.seed(7)
...
for (i in seq_along(alphas)) {
  ...
  predicted_prices[[i]] <- predict(elastic_net[[i]], newx = X_matrix, s = "lambda.min")
...
```
```{r}
fit= cv.glmnet(X_matrix, Y_matrix, alpha = 1/20)
plot(fit)
...
```{r}
fit_final= glmnet(X_matrix, Y_matrix, alpha = 0)
...
predicted_prices<- predict(fit, newx = X_matrix,s=0.02844251)
